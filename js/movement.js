/* Allow movement through thumbsticks */
/* For faster movement, use both thumbstick simultaneously */
/* global AFRAME, THREE */
// Nothing to change here.

AFRAME.registerComponent("oculus-thumbstick-controls", {
  schema: {
    acceleration: { default: 17 },
  ...
  init: function () {
    if (AFRAME.utils.device.isMobileVR()) {
      var rigg = document.querySelector(this.data.rigSelector); 
      // Remove the 'wasd-controls' component
      rigg.removeAttribute("wasd-controls"); 
      // Remove the 'look-controls' component
      rigg.removeAttribute("look-controls");
      
      var entity = document.querySelector('#player');

      // Add the 'wasd-controls' component
      entity.setAttribute('wasd-controls', '');

      // Add the 'look-controls' component
